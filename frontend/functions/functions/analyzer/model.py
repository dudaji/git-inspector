...
class Scores(BaseModel):
    winner: str = Field(
        description="Name of the cloud provider with the highest score"
    )
    ...
    language: float = Field(
        description="Environmental score of repository code"
    )
...
class GeminiAnalysis(BaseModel):
    ...
    directory: str = Field(
        description="The directory name requested by the client"
    )
    result: Result = Field(description="Gemini generated result")
    scores: Scores = Field(
        description="The calculated cloud provider scores from result"
    )
    languages: Dict[str, float] = Field(
        description="Programming language ratio of repository"
    )
